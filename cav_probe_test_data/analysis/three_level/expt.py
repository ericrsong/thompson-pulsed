...
from thompson_pulsed.core.parsers import ni_pci5105

__all__ = ["Experiment", "Parameters", "Data"]
...
class Experiment:
    ...
    def load_sequence(self, file):
        ...
        for i in range(tries):
            try:
                seq = tp.Sequence.load(file, ni_pci5105)
            except tp.SequenceLoadException as err:
...
class Data:
    ...
    def __init__(self, t, cav_runs, atom_runs, params, fi=None, fb=None, cref_runs=None, spcm_runs=None):
        self.t = t
        self.cav_runs = tp.Time_Multitrace(t, cav_runs) \
            if (cav_runs is not None) else None
        self.atom_runs = tp.Time_Multitrace(t, atom_runs) \
        ...
        self.cref_runs = tp.Time_Multitrace(t, cref_runs) \
            if (cref_runs is not None) else None

        self.spcm_runs = tp.Time_Multitrace(t, spcm_runs) \
    ...
    def _seq_cav_probe_mag(self, f_demod = None, avg_shots=True):
        ...
        if avg_shots:
            cav_seqs_mag = tp.Time_Multitrace(cav_runs_mag.t,
    ...
    def track_cav_frequency_iq(self, t_bin, f_demod = None, align = True, avg_sequences = True, \
        ...
        if (self.cref_runs is not None) and use_cref:
            ...
            cav_phasor = tp.MT_Phasor(cav_phasor_raw.t,
        ...
        if int(self.params.t_cav_pulse/t_bin) > 1:
            ...
            phase_flat_aligned = tp.MT_Phase(t_aligned, phase_flat_aligned_vals)
        ...
        cav_freqs = tp.Time_Multitrace(bin_times, cav_freq_vals)
        ...
        if avg_sequences is True:
            ...
            cav_freqs = tp.Time_Multitrace(bin_times,
    ...
    def demod_atom_trace(self, t_align=None, collapse=True, 
        ...
        t_align : float, optional.
            ...
            over 100 ns. If None, function does not perform phase alignment.
            Default is None.
        ...
        filt : str, optional
            Specifies what filter to use for demodulation. Default is "butter".
            
        f_cutoff : float, optional
            Specifies at what frequency to set the cutoff filter for demodulation.
            Default is None.
        ...
        if collapse:
            atom_demod_raw = tp.Time_Multitrace(self.atom_runs.t, V) \
                                .collapse() \
                                .iq_demod(f0, filt=filt, f_cutoff=f_cutoff)
        else:
            atom_demod_raw = tp.Time_Multitrace(self.atom_runs.t, V) \
                                .iq_demod(f0, filt=filt, f_cutoff=f_cutoff)            
        ...
        if t_align is not None:
            di_ref = round(t_avg/self.params.dt)
