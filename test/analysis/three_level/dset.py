...
from .constants import chi, g0689, kappa689, \
    alpha_a, JPerp_per_V # calibrations
...
class Dataset():
    ...
    def process_data(self, t_bin,
        ...
        # plt.figure()
        # plt.plot(peak_finder.t, peak_finder.V)
        # plt.plot(peak_finder.t[peaks], peak_finder.V[peaks],'o')

        t_drive_center = peak_finder.t[ peaks ]
        t_align = t_drive_center - self.params.t_drive/2
        t0 = t_align \
            if (include_pulse_in_traces is True) \
            else (t_drive_center + self.params.t_drive/2)
        ...
        # # Get NB, ND arrays
        cav_probe_N = (cav_freq_shift - f0) * df_to_N # NB + ND/2
        ...
        """
        Postprocessing of Rabi frequency and total drive phase
        """
        ####
        # Use self.postmeasure drive pulse to calibrate drive angle θ0
        ####

        # Use magnitude average to detect position of peak
        post_avg_mag2 = self.postmeasure.demod_atom_trace().mag2().average_over(axis=0)
        t_drive_post_center = post_avg_mag2.t[ np.argmax(post_avg_mag2.V) ]
        t_post_align = t_drive_post_center - self.params.t_drive * 1/2
        ...
        post_avg_phasor = self.postmeasure.demod_atom_trace(t_post_align).average_over(axis=0)
        ...
        ti_pulse = t_drive_post_center - self.params.t_drive * 3/4
        tf_pulse = t_drive_post_center + self.params.t_drive * 3/4
    ...
    def get_label(self, param_type):
        if param_type == "chiN":
            ...
            return( fr"$\theta_0$ = {self.sd.theta0} $\pi$" )
